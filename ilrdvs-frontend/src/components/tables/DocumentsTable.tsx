      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-xs text-slate-500 border-b border-slate-100">
            ...
            <th className="px-4 py-3 font-medium">{t("table.fileName", "File Name")}</th>
            <th className="px-4 py-3 font-medium">{t("table.location", "Location")}</th>
            <th className="px-4 py-3 font-medium">{t("table.recordType", "Record Type")}</th>
            <th className="px-4 py-3 font-medium">{t("table.uploadedBy", "Uploaded By")}</th>
            <th className="px-4 py-3 font-medium">{t("table.uploadDate", "Upload Date")}</th>
            <th className="px-4 py-3 font-medium">{t("table.processing", "Processing")}</th>
            ...
            <th className="px-4 py-3 font-medium">{t("table.verification", "Verification")}</th>
            <th className="px-4 py-3 font-medium text-right">{t("table.actions", "Actions")}</th>
          </tr>
        </thead>
            ...
            <tr key={doc.id} className="hover:bg-slate-50/70 transition-colors">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center justify-end gap-1">
                  <button title={t("common.view", "View")} onClick={() => navigate(`/documents/${doc.id}`)} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-500">
                    <Eye className="h-3.5 w-3.5" />
                  </button>
                  <button title={t("common.process", "Process")} onClick={() => navigate(`/documents/processing/${doc.id}`)} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-500">
                    <Cpu className="h-3.5 w-3.5" />
                  </button>
                  <button title={t("common.verify", "Verify")} onClick={() => navigate(`/verification`)} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-500">
                    <ClipboardCheck className="h-3.5 w-3.5" />
                  </button>
                  <button title={t("common.download", "Download")} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-500">
                    <Download className="h-3.5 w-3.5" />
                  </button>
                  <button title={t("common.more", "More")} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-500">

...
import { Search, SlidersHorizontal, UploadCloud, FileStack } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-navy-900">{t("documents.title", "Document Management")}</h1>
          <p className="text-sm text-slate-500 mt-0.5">{t("documents.subtitle", "Search, filter, and manage all uploaded land record documents.")}</p>
        </div>
      ...
      <Card>
        <div className="p-4 flex flex-wrap items-center gap-3 border-b border-slate-100">
          <div className="flex-1 min-w-[220px]">
            <Input
              ...
              placeholder={t("documents.searchPlaceholder", "Search by document ID, file name, village…")}
              value={search}
          ...
          <div className="w-44">
            <Select value={status} onChange={(e) => { setPage(1); setStatus(e.target.value); }}>
              <option value="">{t("documents.allStatuses", "All Statuses")}</option>
              <option value="uploaded">{t("status.uploaded", "Uploaded")}</option>
        ...
        {items === null ? (
          ...
          <EmptyState
            ...
            title={t("documents.noDocuments", "No documents found")}
            description={t("documents.noDocumentsSub", "Try adjusting your search or filters, or upload a new land record document.")}
            action={<Button size="sm" onClick={() => navigate("/documents/upload")}>{t("documents.upload", "Upload Document")}</Button>}
          />
        ) : (
          <>
